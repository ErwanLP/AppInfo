    ...
    <article>
        ...
        <?php if (isset($_SESSION['SWITCH']) AND ($_SESSION['SWITCH'] == "organisateur" OR $_SESSION['SWITCH'] == "participant" ) AND $_SESSION['ID'] != null) { ?>
            <div class="creerTopic">
                   <a href="ajouterSujet.php?idSouscategorie=2"> Ajouter un sujet</a>
            </div>
        <?php } ?>
        ...
        <?php
        $tab_info_taverne = array();
        $var_tab_info_array = 0;
        $req = $bdd->query('(SELECT organisateur.pseudo,topicforum.* FROM topicforum, souscategorieforum,organisateur WHERE souscategorieforum.ID = 2 
            AND topicforum.id_souscategorie = souscategorieforum.ID
            AND topicforum.id_organisateur = organisateur.id
            )
            UNION 
            (
            SELECT participant.pseudo,topicforum.* FROM topicforum,souscategorieforum,participant WHERE souscategorieforum.ID=2
             AND topicforum.id_souscategorie = souscategorieforum.ID
            AND topicforum.id_participant = participant.id)');
        while ($donnees = $req->fetch()) {
            $tab_info_taverne[$var_tab_info_array][0] = $donnees["nom"];
            $tab_info_taverne[$var_tab_info_array][1] = $donnees["commentaire"];

            $tab_info_taverne[$var_tab_info_array][2] = $donnees["date_creation"];
            $tab_info_taverne[$var_tab_info_array][3] = $donnees["id"];
            $tab_info_taverne[$var_tab_info_array][4] = $donnees["pseudo"];
            $var_tab_info_array++;
        }
        ?>
                    ...
                    <table class="affichageTableau">
                        ...
                        <?php for ($a = 0; $a < count($tab_info_taverne); $a++) {
                            ...
                            <tr class="affichageSujet">
                                <td class="contenuMessage"><a href="commentaireForum.php?idTopic=<?php echo $tab_info_taverne[$a][3] ?>&titreTopic=<?php echo $tab_info_taverne[$a][0] ?>"><?php echo $tab_info_taverne[$a][0]; ?></a></td>
                                <td class="message"><?php echo $tab_info_taverne[$a][1]; ?></td>
                                <td class="auteur"><?php echo $tab_info_taverne[$a][4]; ?></td>
