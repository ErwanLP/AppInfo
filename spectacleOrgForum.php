...
<section>
    ...
    <article>
        ...
        <div class="creerTopic">
               <a href="ajouterSujet.php?idSouscategorie=10"> Ajouter un sujet</a>
        </div>
        ...
        $req = $bdd->query('(SELECT organisateur.pseudo,topicforum.* FROM topicforum, souscategorieforum,organisateur WHERE souscategorieforum.ID = 10 
            AND topicforum.id_souscategorie = souscategorieforum.ID
            AND topicforum.id_organisateur = organisateur.id
            )
            UNION 
            (
            SELECT participant.pseudo,topicforum.* FROM topicforum,souscategorieforum,participant WHERE souscategorieforum.ID=10
             AND topicforum.id_souscategorie = souscategorieforum.ID
        ...
        while ($donnees = $req->fetch()) {
            ...
            $tab_info_spectacle[$var_tab_info_array][3] = $donnees["pseudo"];
            $var_tab_info_array++;
        }
        ?>
