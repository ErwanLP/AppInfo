...
<section>
    ...
    <article>
        ...
        <div class="creerTopic">
              <a href="ajouterSujet.php?idSouscategorie=7"> Ajouter un sujet</a>
        </div>
        ...
        $req = $bdd->query('(SELECT organisateur.pseudo,topicforum.* FROM topicforum, souscategorieforum,organisateur WHERE souscategorieforum.ID = 7 
            AND topicforum.id_souscategorie = souscategorieforum.ID
            AND topicforum.id_organisateur = organisateur.id
            )
            UNION 
            (
            SELECT participant.pseudo,topicforum.* FROM topicforum,souscategorieforum,participant WHERE souscategorieforum.ID=7
             AND topicforum.id_souscategorie = souscategorieforum.ID
