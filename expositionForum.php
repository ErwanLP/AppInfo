...
<?php }?>
       ...
       <?php//SELECT topicforum.* FROM topicforum, souscategorieforum WHERE souscategorieforum.ID = 5
        //    AND topicforum.id_souscategorie = souscategorieforum.ID?>

        <?php
        $tab_info_exposition = array();
        $var_tab_info_array = 0;
        $req = $bdd->query('(SELECT organisateur.pseudo,topicforum.* FROM topicforum, souscategorieforum,organisateur WHERE souscategorieforum.ID = 5 
            AND topicforum.id_souscategorie = souscategorieforum.ID
            AND topicforum.id_organisateur = organisateur.id
            )
            UNION 
            (
            SELECT participant.pseudo,topicforum.* FROM topicforum,souscategorieforum,participant WHERE souscategorieforum.ID=5
             AND topicforum.id_souscategorie = souscategorieforum.ID
        ...
        while ($donnees = $req->fetch()) {
            ...
            $tab_info_exposition[$var_tab_info_array][4] = $donnees["pseudo"];
            $var_tab_info_array++;
        }
        ?>
